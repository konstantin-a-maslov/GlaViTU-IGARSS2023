...
def ResUNetDecoder(
    ...
    outputs = tf.keras.layers.Dense(n_outputs, activation=last_activation, name="output")(x)
...
def ResUNet(
    input_shape, n_outputs, n_steps=4, start_n_filters=64, last_activation="softmax", dropout=0, 
    name="ResUNet", **kwargs
):
    inputs = tf.keras.layers.Input(input_shape)
