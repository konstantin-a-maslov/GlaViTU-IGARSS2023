...
def SETRPUPDecoder(
    ...
    outputs = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation="bilinear", name="output")(conv5)
...
def SETR(
    ...
    n_blocks=12, n_heads=12, dropout=0.1, n_filters=256, name="SETR", **kwargs
):
    inputs = tf.keras.layers.Input(input_shape)
...
def FusionSETR(
    ...
    n_blocks=12, n_heads=12, dropout=0.1, n_filters=256, name="FusionSETR", **kwargs
):
    inputs = []
...
def SETR_B16(
    input_shape, n_outputs, last_activation="softmax", dropout=0.1, 
...
def FusionSETR_B16(
    input_shapes, n_outputs, last_activation="softmax", dropout=0.1, 
